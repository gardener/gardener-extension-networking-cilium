...
var _ = Describe("Network Extension Tests", Label("Network"), func() {
	...
	f.Shoot = encryptedShoot("enc-con-test")

	It("Create Shoot, Test Encrypted Network (Cilium Connectivity), Delete Shoot", Label("good-case"), func() {
		By("Create Shoot")
		...
		shootCiliumIPSecKeysSecret := &corev1.Secret{}
		err = f.ShootFramework.ShootClient.Client().Get(ctx, client.ObjectKey{Namespace: values.HelmDeployNamespace, Name: "cilium-ipsec-keys"}, shootCiliumIPSecKeysSecret)
		Expect(err).NotTo(HaveOccurred())
