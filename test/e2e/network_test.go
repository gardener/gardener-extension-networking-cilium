...
var _ = Describe("Network Extension Tests", Label("Network"), func() {
	...
	It("Create Shoot, Test Networking, Delete Shoot", Label("good-case"), func() {
		...
		err = f.RenderAndDeployTemplate(ctx, f.ShootFramework.ShootClient, templates.NetworkingTestJobName, values)
		gomega.Expect(err).NotTo(gomega.HaveOccurred())
		...
		var succeeded bool
		for {
			err = f.ShootFramework.ShootClient.Client().Get(ctx, kutil.Key(values.HelmDeployNamespace, "network-test"), job)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
